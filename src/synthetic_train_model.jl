...
function main()
    ...
    for epoch in 1:epochs
        ...
        for (batch_nr, (x_batch, y_batch)) in enumerate(train_dataloader)
            ...
            # println("size(stacked_images): ", size(stacked_images))
            # println("typeof(stacked_images): ", typeof(stacked_images))

            # println("size(y_batch): ", size(y_batch))
            # println("typeof(y_batch): ", typeof(y_batch))
