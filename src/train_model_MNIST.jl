...
function main()
    ...
    data_path_test = "data/MNIST"

    model_name = "MNIST_save_nr_$(load_model_nr).jld2"
    ...
    for epoch in 1:epochs
        ...
        loader = get_dataloader_MNIST(data_name, data_path, BATCH_SIZE)
        for (batch_nr, (images, _)) in enumerate(loader)
            ...
            if batch_nr * BATCH_SIZE % evaluate_interval == 0
                loader_test = get_dataloader_MNIST(data_name, data_path_test, BATCH_SIZE)
                ...
                loader_eval = get_dataloader_MNIST(data_name, data_path, BATCH_SIZE)
